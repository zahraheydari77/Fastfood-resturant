...
export default function FoodBox({ foods, addProductToCart }) {
  const foodFilter = (food) => {
    addProductToCart(food)
    toast.success("محصول به سبد خرید اضافه شد!");
  };

  return (
    <>
      <div className="row_grid_food_product">
        {foods.map((food) => {
          return (
            <div key={food.id} className="item_food_grid">
              <div className="Badge_Discount">
                {food.off === "1" ? (
                  <img src="./img/Badge Discount (1).png" />
                ) : (
                  ""
                )}
              </div>
              <div className="img_item_food_grid">
                <img src={food.img} alt="" />
                <div></div>
              </div>
              <div className="info_item_food_grid">
                <h5>{food.title}</h5>
                <p>{food.desc}</p>
                <div className="score_item_food_grid">


                  {
                    Array(5 - food.score).fill(0).map(item => (
                      <FaRegStar className="course-box__star" />
                    ))
                  }

                  {
                    Array(food.score).fill(0).map(item => (
                      <FaStar className="course-box__star" />
                    ))
                  }

                </div>
                <div className="bottom_item_food_grid">
    
                      <span>{food.price.toLocaleString('fa-IR')}</span>
             
                <AnchorLink href="#basket">
                <button
                    onClick={() => {
                      foodFilter(food);
                    }}
